...
@objc protocol CustomImagePickerDelegate {
    func didImagePickerFinishPicking(_ image: UIImage)
    @objc optional func didCancelImagePicking()
    @objc optional func didVideoPickerFinishPicking(_ video: URL)
...
class CustomImagePickerView: NSObject {
    ...
    fileprivate func createViewToCropImage(image: UIImage) {
       ...
       // cancelButton.titleLabel!.shadowColor = UIColor.clear
        cancelButton.setTitleColor(UIColor(red: 0, green: 122/255, blue: 1, alpha: 1.0), for: UIControl.State())
       ...
       // doneButton.titleLabel!.shadowColor = UIColor.clear
        doneButton.setTitleColor(UIColor(red: 0, green: 122/255, blue: 1, alpha: 1.0), for: UIControl.State())
...
extension CustomImagePickerView: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        picker.dismiss(animated: true, completion: nil)
        if let img = info[.editedImage] as? UIImage
        {
            delegate?.didImagePickerFinishPicking(img)
        }
        else if let img = info[.originalImage] as? UIImage {
            delegate?.didImagePickerFinishPicking(img)
        }
        else if let videoURL = info[.mediaURL] as? URL {
            delegate?.didVideoPickerFinishPicking?(videoURL)
        }
    }
}
